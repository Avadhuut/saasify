...
/**
 * Configuration that binds Spring Data JPA repositories to scan entity packages and exposes
 * thread-safe synchronized methods to dynamically register database connections for tenants.
 */
@Configuration
@EnableTransactionManagement
@EnableJpaRepositories(
        basePackages = "com.saasify.user.repository",
...
public class MultiTenantDataSourceConfig {
    ...
    public LocalContainerEntityManagerFactoryBean entityManagerFactory(MultiTenantRoutingDataSource routingDataSource) {
        ...
        em.setPackagesToScan("com.saasify.user.entity");
    ...
    /**
     * Look up tenant schema names dynamically and register a custom database connection pool.
     */
    public synchronized void registerTenantDataSource(String tenantId, MultiTenantRoutingDataSource routingDataSource, DataSource masterDataSource) {
        ...
        try (Connection connection = masterDataSource.getConnection();
            ...
            throw new RuntimeException("Database error during tenant schema configuration query: " + tenantId, e);
        }
        ...
        if (!dbUrl.contains(masterPlaceholder)) {
            throw new IllegalStateException("Database URL configuration is missing '" + masterPlaceholder + "' placeholder.");
        }
        ...
        ds.setPoolName("HikariPool-user-tenant_" + tenantId);
